...
async def register(data: RegisterUserRequest, service: AuthServiceDep) -> AuthResponse:
    try:
        ...
        raise HTTPException(status.HTTP_409_CONFLICT, 'Пользователь с таким email уже существует') from exc
    return AuthResponse(user=user, message='Профиль создан', access_token=service.access_token(user))
...
async def login(data: LoginUserRequest, service: AuthServiceDep) -> AuthResponse:
    ...
    return AuthResponse(user=user, message='Вход выполнен', access_token=service.access_token(user))
