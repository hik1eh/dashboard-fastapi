...
export function DashboardPage() {
  ...
  return <div className="page dashboard-page">
    ...
    <div className="dashboard-grid">
      ...
      <Panel title="Сегодня по расписанию" action={<Link to="/schedule"><ArrowUpRight size={16}/></Link>} className="span-2"><div className="schedule-strip">{data.schedule.filter(s => s.weekday === (new Date().getDay()+6)%7).length ? data.schedule.filter(s => s.weekday === (new Date().getDay()+6)%7).map(item => <div key={item.id}><time>{item.starts_at.slice(0,5)}</time><strong>{item.title}</strong><span>{item.location || 'Без аудитории'}</span></div>) : <Empty text="В расписании на сегодня свободно"/>}</div></Panel>
    </div>
  </div>
}
